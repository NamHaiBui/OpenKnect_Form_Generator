...
import 'package:openknect_form_generator/layout/adaptive_layout_template/service/state_management.dart';
...
Map<Breakpoint, SlotLayoutConfig?> defaultBuildPrimaryNavBarConfig(
    ...
    Map<Breakpoint, Widget> currentBodyWidgets,
    Map<Breakpoint, Widget>? currentSecondaryBodyWidgets,
    Map<Breakpoint, Widget> Function(int selectedIndex) bodyWidgets,
    Map<Breakpoint, Widget> Function(int selectedIndex)? secondBodyWidgets,
    ...
    SlotLayoutConfig slotLayoutConfig = SlotLayout.from(
      ...
      builder: (_) => AdaptiveScaffold.standardNavigationRail(
        ...
        onDestinationSelected: (int newIndex) async {
          ...
          // Update body widgets
          ref.read(bodyWidgetsProvider.notifier).updateBodyWidgets(
              bodyWidgets(ref.read(selectedNavigationProvider)));

          // Update secondary body widgets
          if (secondBodyWidgets != null) {
            ref
                .read(secondaryBodyWidgetsProvider.notifier)
                .updateSecondaryBodyWidgets(
                    secondBodyWidgets(ref.read(selectedNavigationProvider)));
          }
          // ValueNotifier<Map<Breakpoint, Widget>> test = currentBodyWidgets;
          // print('newCurrentBody: $currentBodyWidgets');
          print('selectedNavigation: $newIndex');
          // print('ref: $ref.read(selectedNavigationProvider');
